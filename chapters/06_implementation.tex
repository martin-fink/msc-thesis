...
In \cref{fig:wasmtime-mte-impl} we can see an overview of our implementation of the memory safety extension as implemented in wasmtime using \ac{MTE}.
...
Since the memory belonging to the runtime is tagged with zero, the linear memory is tagged with the tag 1.
...
\end{enumerate}
However, we need to ensure adding an untrusted index to the heap base does not overflow and thus change the tag, allowing code to escape the sandbox.
...
To prevent this, we need to mask out the tag bits that are allocated for the runtime.
...
In \cref{fig:mte-bounds-checks-mem-safety} we see what happens in the case both \ac{MTE} bounds checking and memory safety are enabled.
