...
An instruction $e$ is valid under the context $C$, with $C_\text{memory}$ being used to access a component of the context.
...
In \cref{fig:smallstep-rules}, we extend the small-step reduction rules from the \ac{WASM} paper~\cite{haas2017bringing} in the notation of \citeauthor*{plotkin1981structural}~\cite{plotkin1981structural}.
The lower part of \cref{fig:smallstep-rules} contains the new rules for our new instructions and added rules for load/store instructions that take precedence over the existing ones.
We reuse some of the operators from the rules of the \ac{WASM} paper, most notably the \textbf{trap} operator to signal when a trap occurs.
The new load/store rules ensure that loads and stores trap on tag mismatches.
On the left-hand side, we see the top of the operand stack and the state $s$.
On the right-hand side, we show the resulting state and operating stack after executing the instruction $i$.

We extend the state $s$ with a new tag storage consisting of raw bytes $b^*$.
In this figure, we only show the added or modified components, not the full rules.
